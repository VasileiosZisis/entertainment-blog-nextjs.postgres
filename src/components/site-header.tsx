...
export function SiteHeader() {
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-border bg-background/92 backdrop-blur">
      ...
      <div
        className={`grid border-t border-border transition-[grid-template-rows] duration-200 md:hidden ${
            ...
            {PUBLIC_NAV_ITEMS.map((item) => {
              ...
              return (
                <li key={item.href}>
                  <Link
                    href={item.href}
                    className={`block border-b border-border py-4 text-base font-medium transition-colors last:border-b-0 ${
